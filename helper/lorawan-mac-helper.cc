                                        ...
                                        int k)
{
    ...
    std::vector<int> ncount(7, 0);
    std::vector<int> nmax(7, 0);
    std::vector<std::pair<double, Ptr<Node>>> sorted;
    std::vector<int> datarate(13);
    std::cout << "HELLOAIAOEVNIO\n";
    datarate[12] = 0;
    datarate[11] = 1;
    datarate[10] = 2;
    datarate[9] = 3;
    datarate[8] = 4;
    datarate[7] = 5;
    std::vector<Ptr<Node>> access;
    int size = 0;
    for (auto j = endDevices.Begin(); j != endDevices.End(); ++j)
    {
        Ptr<Node> object = *j;
        access.push_back(object);
        Ptr<LogDistancePropagationLossModel> loss =
            object->GetObject<LogDistancePropagationLossModel>();
        if (loss)
        {
            double system_loss = loss->GetPathLossExponent();
            sorted.push_back({system_loss, object});
        }
        else
        {
            double minPathLoss = 80;
            double maxPathLoss = 150;
            std::random_device rd;
            std::mt19937 gen(rd());

            // Define a uniform distribution for generating values within the specified range
            std::uniform_real_distribution<> dis(minPathLoss, maxPathLoss);
            sorted.push_back({dis(gen),object});
        }
        size++;
    }
    std::sort(sorted.begin(), sorted.end());
    //std::cout << "HELLOAIAOEVNIO\n";
    NodeContainer sortedDevices;
    for (int i = 0; i < size; i++)
    {
        sortedDevices.Add(sorted[i].second);
    }

    double T = 0.1; // Example value for T and pmac
    double pMac = 0.5;
...
start:
    ...
    {
        ncount[i] = 0;
        double max = (1.0 / (2.0 * log(1 - (T / pMac)))) + 1;
        nmax[i] = max;
        ...
        for (int group = 0; group < k; group++)
        {
            ...
            for (auto j = sortedDevices.Begin(); j != sortedDevices.End(); ++j)
            {
                Ptr<Node> object = *j;
                ...
                double currentRxPower = channel->GetRxPower(14, position, bestGatewayPosition);
                //std::cout<<currentRxPower<<"\n"<<"hello";
                // Compute the power received from the current gateway

                if (currentRxPower > 124.5)
                    sfmin = 7;
                else if (currentRxPower > 127)
                    sfmin = 8;
                else if (currentRxPower > 129.5)
                    sfmin = 9;
                else if (currentRxPower > 132)
                    sfmin = 10;
                else if (currentRxPower > 134.5)
                    sfmin = 11;
                else
                    sfmin = 12;
            }
            sftemp = sfmin;
                ...
                for (auto j = sortedDevices.Begin(); j != sortedDevices.End(); ++j)
                {
                    ...
                    Ptr<ClassAEndDeviceLorawanMac> mac =
                        loraNetDevice->GetMac()->GetObject<ClassAEndDeviceLorawanMac>();
                    NS_ASSERT(mac);
                    mac->SetDataRate(datarate[sftemp]);
                    sfQuantity[sftemp - 7]++;

                    test:
                        if (ncount[sftemp] < nmax[sftemp])
                        {
                            mac->SetDataRate(datarate[sftemp]);
                            sfQuantity[sftemp - 7]++;
                            ncount[sftemp]++;
    ...
    /* for (auto j = endDevices.Begin(); j != endDevices.End(); ++j)
     {
         ...
         Ptr<GatewayLoraPhy> gatewayPhy = gatewayLoraNetDevice->GetPhy
